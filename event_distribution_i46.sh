...
#########################################################################
# This script gives the event distribution for a range of dates by date #
...
HDFS_COMMAND_2="/*.evt.gz 2> /dev/null | awk -F, '/e_type/{i=index(\$2, \":\");m=substr(\$2, i+2, length(\$2)-i-2); ++c[m];} END{PROCINFO[\"sorted_in\"] = \"@val_num_desc\";for(j in c) print j, c[j]}'"
...
# check date
check_date_format() {
  [[ ! $(date "$2" -d "$1" > /dev/null 2>&1) ]] && exit_program 3
}
